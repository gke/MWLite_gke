...
void computeAltitudeControl(void) {
  ...
  if (updateAltitude()) {
    ...
    P = Limit1((conf.pid[ALTITUDE].Kp * AltE >> 7), 150);

    AltitudeIntE += (conf.pid[ALTITUDE].Ki * AltE) >>6;
...
#define USE_ROC_FILTER
    ...
    D = Limit1(conf.pid[ALTITUDE].Kd * ROC >> 4, 150);
    ...
    AltE = Limit1(desiredAltitude - relativeAltitude, 300);
    desiredROC = (AltE * conf.pid[ALTITUDE].Kp) >> 7;
    desiredROC = constrain(desiredROC, 100, 200);

    ROC = SmoothAltDerivative(relativeAltitude) * dTR;
    desiredAcc = desiredROC - ((ROC * conf.pid[ALTITUDE].Kd) >> 4);
