...
ISR(USART1_RX_vect)  {
  if (rxEnabled[1]) 
    rxAppendToBuff(UDR1, 1); 
} // USART1_RX_vect

void txUARTData(uint8_t port) {
...
void serialEnd(uint8_t port) {
  ...
  rxEnabled[port] = false;
  rxHead[port] = rxTail[port] = 0;
...
uint8_t serialPeek(uint8_t port) {
  ...
  if ((rxHead[port] != rxTail[port])) return c; 
  else return 0;
}
#endif
