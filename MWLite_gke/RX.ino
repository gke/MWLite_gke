...
void computeRC(void) { // overkill?
  static uint16_t rcValue[RC_CHANS][4], rcValueMean[RC_CHANS];
  ...
  for (chan = 0; chan < RC_CHANS; chan++) {
    rcValue[chan][rcIndex] = readRawRC(chan);
    rcValueMean[chan] = 0;

    for (a=0;a<4;a++) 
      rcValueMean[chan] += rcValue[chan][a];

    rcValueMean[chan]= (rcValueMean[chan] + 2) >> 2;
    if ( rcValueMean[chan] < (uint16_t)rcData[chan] - 3)
      rcData[chan] = rcValueMean[chan] + 2;
