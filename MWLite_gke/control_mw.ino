...
void computeControl(void) { // Alex Khoroshko  http://www.multiwii.com/forum/viewtopic.php?f=8&t=3671&start=10#p37387
  ...
  for(axis = ROLL; axis <= YAW; axis++) {
    ...
    RateD = (SmoothRateDerivative(axis, RateE) * conf.D8[axis]) >> 5; // forget dt scaling
...
void computeControl(void) {
  ...
  for (axis = ROLL; axis < YAW; axis++) {
    ...
    P = ((int32_t)Desired * conf.P8[axis]) >> 6;
  ...
  // Yaw
  Desired = ((int32_t)rcCommand[YAW] * (conf.yawRate * 2 + 30)) >> 5;
  ...
  if (abs(Desired) > 50) 
    ...
    I = RateIntE[YAW]  = Limit1(RateIntE[YAW] + (int32_t)RateE * conf.I8[YAW], (int32_t)RATE_MAX_I << 13);
    I = RateIntE[YAW] >> 13;
  }
